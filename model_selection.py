# -*- coding: utf-8 -*-
"""
Created on Thu Sep 22 17:40:19 2022
...
Has_HD = logmodel.predict_proba(X_test)
Has_HD = [x[1] for x in Has_HD] 

print("\n-----Out of sample test: Logistic regression")
...
# recall: 0.736
# brier_score_loss: 0.16092468082203465
...
# recall: 0.716
# brier_score_loss: 0.1676706415767302
...
clf_svm = SVC(kernel='rbf', random_state=42, C=100, gamma= 0.0001)
clf_svm.fit(X_train_scaled, y_train)
...
Has_HD = classifier.predict_proba(X_test_scaled_df)
Has_HD = [x[1] for x in Has_HD] 
...
ideal_ccp_alpha = alpha_results[(alpha_results['alpha'] >  0.0045) 
                                & 
...
Has_HD = clf_dt_pruned.predict_proba(X_test)
Has_HD = [x[1] for x in Has_HD] 
...
plot_confusion_matrix(rf, X_test, y_test, display_labels=["Does not have HD", "Has HD"])

print("\n-----Out of sample test: Random Forest Classifier ")



predicted = rf.predict(X_test) 
Has_HD = rf.predict_proba(X_test)
Has_HD = [x[1] for x in Has_HD] 
...
# recall: 0.684
# brier_score_loss: 0.1746505491294791
...
plot_confusion_matrix(clf_gbc, X_test, y_test, display_labels=["Does not have HD", "Has HD"])
...
# recall: 0.712
# brier_score_loss: 0.16574191307979022
...
predicted = clf_gbc.predict(X_test) 
Has_HD = clf_gbc.predict_proba(X_test)
Has_HD = [x[1] for x in Has_HD] 
...
# # Dump the model in wb 
# pickle.dump(clf_xgb, open('model.pkl','wb'))
